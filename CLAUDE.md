...
- **`keys.py`** — `KeyPool`: multi-key failover + cooldown for upstream gateway keys (`AI_GATEWAY_API_KEY`, `AI_GATEWAY_API_KEY_1..20`). Used via `_upstream_pooled()` in `server.py`.
