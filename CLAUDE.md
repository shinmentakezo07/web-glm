...
The `baicode_install.sh` script at the repo root is an unrelated third-party installer — do not modify it when working on the proxy.
...
uv run pytest tests/                                 # unit tests for converter.py
uv run pytest tests/test_converter.py::TestOpenAIToV3 # single test class
...
## Two-layer split (preserve it)

- **`converter.py`** — pure functions, no I/O. OpenAI ↔ AI SDK v3 translation + `validate_tool_history()`. Add request/response-shape work here, with tests in `tests/test_converter.py`.
- **`server.py`** — FastAPI + HTTP transport (shared `httpx.AsyncClient` in `lifespan`, HTTP/2 on). Add routes or transport changes here.
...
The upstream Gateway returns 400/503 if these are violated. They are NOT linter-enforceable; treat them as hard rules when editing `converter.py` or `server.py`:
...
- The fx headers (`User-Agent: fx/0.0.3`, `ai-gateway-protocol-version: 0.0.1`, `ai-language-model-specification-version: 4`, `ai-language-model-id: <model>`, `HTTP-Referer`, `X-Title`) **MUST match** what the fx CLI sends. `_v3_headers()` in `server.py` is the single source of truth — edit it there, not in callers.
- HTTP/2 **MUST** stay enabled on the shared `httpx.AsyncClient` (set in `lifespan`).
- Tool-call wire format (fx): assistant tool calls are content parts `{type: "tool-call", toolCallId, toolName, input}` where `input` is the **raw JSON object**, not a string. Top-level `toolCalls` is NOT the fx shape (but `converter.py` accepts it for robustness).
...
- New AI SDK / OpenAI parameter → add the helper in `converter.py` and pass it through in `openai_to_v3()`; add a `tests/test_converter.py` case asserting the field appears in the v3 body.
